...
import torch
from lida.domain.ports import IInfographicsGenerator

logger = logging.getLogger("lida")


class Infographer(IInfographicsGenerator):
    ...
    def load_model(self) -> None:
        ...
        from peacasso.generator import ImageGenerator
        from peacasso.datamodel import ModelConfig

        if self.model_config is None:
            self.model_config = ModelConfig(device="cuda", model="runwayml/stable-diffusion-v1-5", revision="main")
        self.model = ImageGenerator(model_config=self.model_config)

    def generate(
        self,
        visualization: Union[torch.FloatTensor, Any, str],
        n: int,
        style_prompt: Union[str, List[str]] = "line art pastel",
        return_pil: bool = True,
    ) -> List[Any]:
        """Generate a an infographic, given a visualization and style"""

        # Lazy imports to support optional dependency
